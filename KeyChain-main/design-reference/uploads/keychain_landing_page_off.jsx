...
const KeychanMascot = ({ className = "", size = 400, color }) => (
  <svg viewBox="0 0 280 660" width={size * 0.42} height={size} className={className} fill="none" xmlns="http://www.w3.org/2000/svg" style={{ color }}>
    <g transform="translate(140, 0)" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round">
      {/* HEAD — round cat head from behind, symmetrical rounded ear bumps */}
      <path d="
        M 0, 152
        Q -26, 150  -42, 138
        Q -56, 124  -58, 108
      ...
      {/* BOW LEFT LOOP */}
      <path d="
        M -28, 152  Q -52, 138  -78, 134
        Q -108, 130 -122, 144  Q -138, 160 -124, 178
        Q -110, 194 -82, 192  Q -56, 188  -36, 178
        Q -24, 168  -18, 158
      " strokeWidth="2.8"/>
      <path d="
        M -30, 156  Q -48, 148  -68, 145
        Q -90, 142  -102, 152  Q -114, 164 -102, 176
        Q -88, 186  -64, 182  Q -44, 176  -32, 166
      ...
      {/* BOW KNOT */}
      <path d="
        M 0, 146  Q 13, 156  14, 170  Q 14, 184  7, 196
        Q 3, 202   0, 204  Q -3, 202  -7, 196
        Q -14, 184 -14, 170  Q -13, 156  0, 146  Z
      " strokeWidth="2.5"/>
      <path d="
        M 0, 156  Q 6, 163  6, 173  Q 6, 183  3, 191
        Q 1, 195  0, 196  Q -1, 195 -3, 191
        Q -6, 183 -6, 173  Q -6, 163  0, 156  Z
      " strokeWidth="1" opacity="0.3"/>

      {/* BODY LEFT → TAIL */}
      <path d="
        M -5, 204  Q -14, 234  -18, 266  Q -22, 298  -14, 324
        Q -4, 346    10, 358  Q 18, 368   15, 388
        Q 10, 414   -2, 438  Q -22, 468  -40, 498
        Q -60, 528  -52, 556  Q -42, 580  -20, 590
        Q 2, 598    20, 588  Q 36, 576   38, 556
        Q 38, 538   26, 528  Q 12, 520   2, 532
        Q -8, 546   -2, 564  Q 4, 580    18, 583
      " strokeWidth="2.8"/>

      {/* BODY RIGHT */}
      <path d="
        M 5, 204  Q 14, 234   18, 266  Q 22, 298   14, 324
        Q 4, 346    -10, 358  Q -16, 368  -14, 386
        Q -10, 404  -4, 418  Q 2, 430    6, 438
      " strokeWidth="2.8"/>

      {/* KEY */}
      <path d="
        M 18, 583  L 22, 596  L 20, 602
        L 34, 602  L 34, 636  L 20, 636  L 20, 602
      " strokeWidth="2.2"/>
...
/* ═══════════════════════════════════════════════
   ...
   Stars, moons, constellations, tarot motifs
   ═══════════════════════════════════════════════ */
...
/* Background scene with celestial + tarot elements */
  ...
  return (
    <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, pointerEvents: "none", overflow: "hidden" }}>
      <svg width="100%" height="100%" viewBox="0 0 1440 900" preserveAspectRatio="xMidYMid slice" style={{ position: "absolute", top: 0, left: 0 }}>
        ...
        {/* Large decorative circles — porcelain plate rims */}
        ...
        {/* Stars — 6-pointed (smaller, dot-like) */}
        ...
        {/* Constellations — small connected dot patterns */}
        ...
        {/* Fine orbital lines — celestial feel */}
        ...
        {/* Tiny scattered dots — star field */}
        {[
      ...
      <div style={{
        ...
        background: theme.accent, opacity: isDark ? 0.04 : 0.03,
        filter: "blur(100px)",
      }}/>
      <div style={{
        position: "absolute", bottom: "15%", right: "15%",
        width: 350, height: 350, borderRadius: "50%",
        background: theme.accent, opacity: isDark ? 0.06 : 0.04,
        filter: "blur(120px)",
      }}/>
      <div style={{
        position: "absolute", top: "50%", left: "55%",
        width: 200, height: 200, borderRadius: "50%",
        background: theme.accent, opacity: isDark ? 0.03 : 0.02,
        filter: "blur(80px)",
      }}/>
    </div>
  );
...
/* ═══════════════════════════════════════════════
   MAIN LANDING PAGE
   ═══════════════════════════════════════════════ */
export default function KeyChainLanding() {
  ...
  const theme = {
    ...
    text: isDark ? "#F5F2ED" : "#1A1A1A",
    textSecondary: isDark ? "#9E9E9E" : "#6B6B6B",
  ...
  return (
    <div style={{ background: theme.bg, color: theme.text, minHeight: "100vh", fontFamily: "'Inter', 'Helvetica Neue', system-ui, sans-serif", transition: "background 0.5s ease, color 0.5s ease", overflowX: "hidden" }}>
      ...
      <style>{`
        ...
        .feature-card:hover { transform: translateY(-4px) !important; box-shadow: 0 8px 32px ${isDark ? 'rgba(59,130,246,0.12)' : 'rgba(30,58,138,0.1)'} !important; border-color: ${theme.accent} !important; }
        .cta-btn { transition: all 0.25s ease; }
        .cta-btn:hover { transform: translateY(-2px); box-shadow: 0 6px 24px ${isDark ? 'rgba(59,130,246,0.3)' : 'rgba(30,58,138,0.25)'}; }
        .ghost-btn { transition: all 0.25s ease; }
        .ghost-btn:hover { background: ${theme.accentLight} !important; border-color: ${theme.accent} !important; }
        .theme-toggle { transition: all 0.3s ease; }
        .theme-toggle:hover { background: ${theme.accentLight} !important; transform: scale(1.08); }
        .step-circle { transition: all 0.3s ease; }
        .step-col:hover .step-circle { background: ${theme.accent} !important; color: #fff !important; border-color: ${theme.accent} !important; }
      ...
      {/* ═══════════════════════════════════════════
           HERO SECTION
         ═══════════════════════════════════════════ */}
      <section style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", position: "relative", padding: "60px 24px 100px", overflow: "hidden" }}>
        
        <CelestialBackground theme={theme} isDark={isDark} />

        {/* Top label */}
        <div style={{
          animation: "fadeInUp 0.8s ease-out both", animationDelay: "0.2s",
        ...
        {/* Mascot */}
        <div style={{
          animation: "fadeInUp 1s ease-out both", animationDelay: "0.4s",
          position: "relative", zIndex: 1,
        }}>
          <div style={{ color: theme.accent, animation: "float 6s ease-in-out infinite", display: "flex", justifyContent: "center" }}>
            <KeychanMascot size={160} color={theme.accent} />
          </div>
        </div>

        {/* Main title */}
        <div style={{ animation: "fadeInUp 1s ease-out both", animationDelay: "0.6s", position: "relative", zIndex: 1 }}>
          <h1 style={{
        ...
        {/* Tagline */}
        <p style={{
          animation: "fadeInUp 1s ease-out both", animationDelay: "0.8s",
          fontFamily: "'Playfair Display', Georgia, serif",
        ...
        {/* CTAs */}
        <div style={{
          animation: "fadeInUp 1s ease-out both", animationDelay: "1s",
      ...
      <section
        ...
        style={{ ...sectionStyle("features"), padding: "120px 24px", maxWidth: 1100, margin: "0 auto" }}
      >
      ...
      {/* ═══ MARQUEE DIVIDER ═══ */}
      <div style={{ overflow: "hidden", padding: "44px 0", borderTop: `1px solid ${theme.border}`, borderBottom: `1px solid ${theme.border}` }}>
      ...
      {/* ═══════════════════════════════════════════
           HOW IT WORKS — Horizontal, rounded
         ═══════════════════════════════════════════ */}
      <section
        id="howItWorks"
        ref={(el) => (sectionRefs.current["howItWorks"] = el)}
        style={{ ...sectionStyle("howItWorks"), padding: "120px 24px", maxWidth: 1100, margin: "0 auto" }}
      >
        ...
        {/* Steps — horizontal with connected circles */}
        <div style={{ position: "relative" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 24, position: "relative", zIndex: 1 }}>
            {steps.map((step) => (
              <div key={step.num} className="step-col" style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", cursor: "default" }}>
                {/* Circle with number */}
      ...
      <section
        ...
        style={{ ...sectionStyle("techStack"), padding: "80px 24px 120px", maxWidth: 900, margin: "0 auto" }}
      >
